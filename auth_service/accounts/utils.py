...
def convert_trial_to_active(profile, plan_name=None, mark_converted=True):
    """
    Convert trial to active subscription.
    This is called by external systems (frontend payment logic).
    
    Args:
        profile: User profile
        plan_name: Optional - plan being converted (uses subscription plan if None)
        mark_converted: Whether to mark trial as converted (default True)
    ...
    if subscription.status != SubscriptionStatus.TRIALING:
        return False, f"Subscription is not on trial (current status: {subscription.status})", None
    
    # FREE_ESIGN should never be in trial status
    if subscription.plan_name == PlanName.FREE_ESIGN:
        return False, "FREE_ESIGN plan cannot have trial status (it's free for life)", None
    
    current_time = now()
    trial_plan = plan_name or subscription.plan_name
    
    # Check if trial has already expired
    is_early_conversion = current_time < subscription.trial_end
    
    # Calculate days used for reporting
    days_used = (current_time - subscription.start_date).days
    
    # Update subscription to active
    subscription.status = SubscriptionStatus.ACTIVE
    subscription.trial_end = None
    
    # Set next billing period (30 days from now)
    ...
    if mark_converted:
        try:
            trial = TrialHistory.objects.get(
                ...
                ended_at__isnull=True  # Still active trial
            )
            trial.ended_at = current_time
            trial.converted_to_subscription = True
            trial.conversion_date = current_time
            trial.early_conversion = is_early_conversion
            trial.save()
        except TrialHistory.DoesNotExist:
            # Trial history not found - log this but don't fail
            pass
    
    if is_early_conversion:
        return True, f"Trial converted to active subscription early (after {days_used} days)", subscription
    else:
        return True, "Trial converted to active subscription at expiration", subscription
